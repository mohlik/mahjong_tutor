...
class IgApi extends SocialApi{
	...
	get_current_tournament(on_complete) {	
		FBInstant.getTournamentAsync().then(tournament => {
			...
			console.log('gct', (current_time < tournament.getEndTime()), tournament.getID(), current_time, tournament.getEndTime(), current_time - tournament.getEndTime())
			if (current_time >= tournament.getEndTime()) on_complete(false);
			else {
				let t_id = tournament.getID();
				if (String(t_id) != String(game_data['user_data']['ig_tournament']['context_id']))
					game_request.request({'update_ig_tournament': true, 'context_id': String(t_id)}, (obj)=> {});
		...
		.catch(err => {
			...
			if (t_id) FBInstant.tournament.joinAsync(t_id).then(()=> {
				...
				game_request.request({'update_ig_tournament': true, 'context_id': 0}, (obj)=> {});
